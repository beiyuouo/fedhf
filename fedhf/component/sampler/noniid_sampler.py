...
class NonIIDSampler(BaseSampler):
    ...
    def sample(self, train_dataset, test_dataset=None):
        ...
        sampler_num_samples = self.args.sampler_num_samples if self.args.get("sampler_num_samples") is not None else 1
        sampler_num_samples = int(sampler_num_samples)
        sampler_num_classes = self.args.sampler_num_classes if self.args.get("sampler_num_classes") is not None else 1
        ...
        unbalance_rate = 1.0 if self.args.get("sampler_unbalance_rate") is None else self.args.sampler_unbalance_rate
        unbalance_rate = float(unbalance_rate)

        for i in range(self.args.num_clients):
            rand_set = set(np.random.choice(idx_shard, sampler_num_classes, replace=False))
            ...
            for rand_item in rand_set:
                if unbalance_flag == 0:
                    client_data_train[i] = np.concatenate(
                        (
                            client_data_train[i],
                            idxs[sampler_num_samples * rand_item : int(sampler_num_samples * (rand_item + 1))],
                        ),
                        axis=0,
                    )
                    client_label = np.concatenate(
                        (
                            client_label,
                            labels[sampler_num_samples * rand_item : int(sampler_num_samples * (rand_item + 1))],
                        ),
                        axis=0,
                    )
                else:
                    client_data_train[i] = np.concatenate(
                        (
                            ...
                            idxs[
                                sampler_num_samples
                                * rand_item : int(sampler_num_samples * (rand_item + unbalance_rate))
                            ],
                        ),
                        axis=0,
                    )
            ...
            if test_dataset is not None:
                for label in client_label_set:
                    client_data_test[i] = np.concatenate(
                        (client_data_test[i], idxs_test[int(label) * num_items_test : int(label + 1) * num_items_test]),
                        axis=0,
                    )

        if test_dataset is None:
            return [ClientDataset(train_dataset, list(client_data_train[i])) for i in range(self.args.num_clients)]
        else:
            return [
                (
